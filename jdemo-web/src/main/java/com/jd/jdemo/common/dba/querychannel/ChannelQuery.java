package com.jd.jdemo.common.dba.querychannel;
...
import com.jd.jdemo.common.dba.domain.QueryParameters;
...
public abstract class ChannelQuery<E extends ChannelQuery> {
    ...
    public <T> Page<T> pagedList() {
        return new Page<T>(query.getFirstResult(), queryResultCount(), query.getMaxResults(), query.list());
    }
    ...
    public long queryResultCount() {
        ...
        if (builder.containsGroupByClause()) {
            List rows = createBaseQuery(builder.removeOrderByClause()).setParameters(query.getParameters()).list();
            return rows == null ? 0 : rows.size();
        } else {
            Number result = (Number) createBaseQuery(builder.buildQueryStringOfCount()).setParameters(
                    query.getParameters()).singleResult();
