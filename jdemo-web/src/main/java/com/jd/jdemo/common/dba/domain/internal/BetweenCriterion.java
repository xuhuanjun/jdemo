...
public class BetweenCriterion extends BasicCriterion {
    ...
    public String toQueryString() {
        return String.format("%s between %s and %s", getPropNameWithAlias(), getParamNameWithColon() + "_from",
                getParamNameWithColon() + "_to");
    }

    public NamedParameters getParameters() {
        return NamedParameters.create().add(getParamName() + "_from", this.from).add(getParamName() + "_to", this.to);
    }
    ...
    public boolean equals(Object other) {
        ...
        return new EqualsBuilder().append(getPropName(), that.getPropName()).append(getFrom(), that.getFrom())
